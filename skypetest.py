...
def CreateDTMFTones(dialstr,parSkype,callID):
    for count in range(0,len(dialstr)):
        parSkype.SendCommand(parSkype.Command('SET CALL ' + str(callID) + ' DTMF ' + str(dialstr[count])))
        time.sleep(1)            
...
if call.Status == Skype4Py.clsInProgress:
        print 'Waiting to send DTMF'
        time.sleep(10)
        skype.SendCommand(skype.Command('SET CALL ' + str(call.Id) + ' DTMF ' + '1'))
        time.sleep(5)
        skype.SendCommand(skype.Command('SET CALL ' + str(call.Id) + ' DTMF ' + '1'))
        time.sleep(7)
        CreateDTMFTones('1234567890',skype,call.Id)
        #skype.SendCommand(skype.Command('SET CALL ' + str(call.Id) + ' DTMF ' + '9276'))
sys.exit()
